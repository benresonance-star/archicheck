...
- Optional `outputKind` on a checklist item places a job-authored or live-edited check on the document lens when the bundled id map does not know the id. Older ZIPs without `outputKind` still validate.
- Template and impact `changes` may set optional `kind` (`wording`, `add`, `edit`, `remove`, `move`) plus stage, deliverable, typology, resource and index before/after fields. Missing `kind` is wording, so older impact notices still validate.
